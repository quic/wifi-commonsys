...
public class QtiSupplicantStaIfaceHalAidlImpl implements IQtiSupplicantStaIfaceHal {
    ...
    private class SupplicantVendorStaIfaceCallback extends ISupplicantVendorStaIfaceCallback.Stub {
        ...
        public String getInterfaceHash() {
            return ISupplicantVendorStaIfaceCallback.HASH;
        }

        @Override
        public int getInterfaceVersion() {
            return ISupplicantVendorStaIfaceCallback.VERSION;
        }
    }
    ...
    public boolean setupVendorIface(@NonNull String ifaceName) {
        synchronized (mLock) {
            ...
            try {
                ...
                serviceBinder.linkToDeath(mSupplicantVendorDeathRecipient, /* flags= */  0);
                ISupplicantVendorStaIfaceCallback callback = new SupplicantVendorStaIfaceCallback();
                vendor_iface.registerSupplicantVendorStaIfaceCallback(callback);
            } catch (RemoteException e) {
                handleRemoteException(e, methodStr);
                return false;
            }
